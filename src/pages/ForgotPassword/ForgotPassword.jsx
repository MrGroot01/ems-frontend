...
export default function ForgotPassword() {
  ...
  const handleStep1 = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setStep(2);
        setOtp(['','','','','','']);
        startTimer();           // ← start countdown when OTP step opens
      }, 1400);
    } catch (ex) {
  ...
  const handleStep2 = async (e) => {
    ...
    if (timerDone) { setErr('OTP expired. Please resend.'); return; }
    setLoading(true); clearMsgs();
    try {
  ...
  const timerPct  = (timer / OTP_TIMER) * 100;           // for SVG arc
  const timerColor = timer > 15 ? '#10b981' : timer > 8 ? '#f59e0b' : '#ef4444';
  // SVG circle math
  const R  = 16;
  const C  = 2 * Math.PI * R;  // circumference ≈ 100.5
  const strokeDash = (timerPct / 100) * C;
          ...
          <form onSubmit={handleStep2}>
            ...
            <div className="otp-timer-wrap">
              ...
              <div className="otp-timer-label">
                {timerDone
                  ? <span style={{color:'#f87171'}}>OTP expired</span>
                  : <span style={{color:'rgba(255,255,255,.4)'}}>
                      expires in <strong style={{color:timerColor}}>{timer}s</strong>
                    </span>
                }
              </div>
            </div>
            ...
            <div className={`otp-row ${timerDone ? 'expired' : ''}`}>
              {otp.map((d,i) => (
                <input key={i} ref={el => refs.current[i]=el}
                  ...
                  disabled={timerDone}
                  style={{
                    borderColor: timerDone ? 'rgba(239,68,68,.3)' : '',
                    opacity: timerDone ? 0.5 : 1,
                  }}
                />
              ))}
            </div>
